...
class DatabaseTest {
    ...
    @Before
    fun setup() {
        // ✅ Use ApplicationProvider to get context in unit tests
    ...
    fun testDeleteExcursion() = runBlocking {
        ...
        val vacationId = vacationDao.insertAll(vacation)[0] // ✅ Get inserted ID
        ...
        val excursions = excursionDao.getExcursionsForVacation(vacationId)
        assert(excursions.isNotEmpty()) { "No excursions found!" } // Ensure there are excursions
        val randomExcursion = excursions.random() // ✅ Pick a random excursion
